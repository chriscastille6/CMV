...
###In-role and organizational citizenship behavior 
  @Williamsalternativeapproachmethod1994 In-Role Performance Behavior (IRB) and Organizational Citizenship Behavior (OCB) scales, the latter of which includes OCBI (OCB directed at the individual) and OCBO (OCB directed at the organization), were used as dependent variables. The items reflect a supervisor's perspective, but were adjusted for this study to be self-referent. Example items (and Cronbach alphas) for each respctive scale: "I perform tasks that are expected of me" ($\alpha$ = `r apa(IRB.alpha,2,T)`), "I take a personal interest in other employees" ($\alpha$ = `r apa(OCBI.alpha,2,T)`), and "I conserve and protect organizational property" ($\alpha$ = `r apa(OCBO.alpha,2,T)`).^[Two other criteria measures were included in this study and study 3. However, we have decided to focus on IRB and OCB because these variables were measured consistently across each of the three studies].
...
method <- cfa(model = method.cfa, ordered = c("PA1", "PA2", "PA3", "PA4", "PA5", "PA6", "PA7", "PA8", "PA9", "PA10", 
                                      "NA1", "NA2", "NA3", "NA4", "NA5", "NA6", "NA7", "NA8", "NA9", "NA10", 
                                      "CM1", "CM2", "CM3", "CM4", "CM5", "CM6", "CM7", "CM8"), 
                                      data = data1, parameterization = "theta", estimator = "dwls", information = "expected")
...
data1$PP2[data1$PP2==1] <- 2
data1$PP6[data1$PP6==1] <- 2
data1$PP10[data1$PP10==1] <- 2
data1$IRB2[data1$IRB2==1] <- 2
data1$IRB3[data1$IRB3==1] <- 2
data1$IRB4[data1$IRB4==1] <- 2
data1$OCBI1[data1$OCBI1==1] <- 2
data1$OCBI2[data1$OCBI2==1] <- 2
data1$OCBI7[data1$OCBI7==1] <- 2
data1$OCBO2[data1$OCBO2==1] <- 2
data1$OCBO4[data1$OCBO4==5] <- 4
data1$OCBO6[data1$OCBO6==1] <- 2
data1$OCBO7[data1$OCBO7==1] <- 2
...
OCBO ~ c(0, 0)*1
'
initial <- cfa(initial.cfa, ordered = c("PP1", "PP2", "PP3", 
                  ...
                  group = "COND", data = data1, estimator = "DWLS", parameterization = "theta", information = "expected", std.lv=TRUE)
...
OCBO ~ c(0, 0)*1
'
baseline <- cfa(baseline.cfa, ordered = c("PP1", "PP2", "PP3", 
                                              "PP4", "PP5", "PP6", "PP7", "PP8", "PP9", "PP10", 
                                              "IRB1", "IRB2", "IRB3", "IRB4", "IRB5", "IRB6", "IRB7", "OCBI1",
                                              "OCBI2", "OCBI3", "OCBI4", "OCBI5", "OCBI6", "OCBI7", "OCBO1", 
                                              "OCBO2", "OCBO3", "OCBO4", "OCBO5", "OCBO6", "OCBO7"),
...
OCBO ~ c(0, 0)*1
'
methodi <- cfa(methodi.cfa, ordered = c("PP1", "PP2", "PP3", 
                                              "PP4", "PP5", "PP6", "PP7", "PP8", "PP9", "PP10", 
                                              "IRB1", "IRB2", "IRB3", "IRB4", "IRB5", "IRB6", "IRB7", "OCBI1",
                                              "OCBI2", "OCBI3", "OCBI4", "OCBI5", "OCBI6", "OCBI7", "OCBO1", 
                                              "OCBO2", "OCBO3", "OCBO4", "OCBO5", "OCBO6", "OCBO7"),
...
##Method factors
PosAff =~ c(1,1)*PA + c(papp1a, papp1a)*PP1 + c(papp2a, papp2a)*PP2 + c(papp3a,papp3a)*PP3 + c(papp4a,papp4a)*PP4 + c(papp5a,papp5a)*PP5 + c(papp6a,papp6a)*PP6 + c(papp7a,papp7a)*PP7 + c(papp8a,papp8a)*PP8 + c(papp9a,papp9a)*PP9 + c(papp10a,papp10a)*PP10 + c(pairb1a,pairb1a)*IRB1 + c(pairb2a,pairb2a)*IRB2 + c(pairb3a,pairb3a)*IRB3 + c(pairb4a,pairb4a)*IRB4 + c(pairb5a,pairb5a)*IRB5 + c(pairb6a,pairb6a)*IRB6 + c(pairb7a,pairb7a)*IRB7 + c(paocbi1a,paocbi1a)*OCBI1 + c(paocbi2a,paocbi2a)*OCBI2 + c(paocbi3a,paocbi3a)*OCBI3 + c(paocbi4a,paocbi4a)*OCBI4 + c(paocbi5a,paocbi5a)*OCBI5 + c(paocbi6a,paocbi6a)*OCBI6 + c(paocbi7a,paocbi7a)*OCBI7 + c(paocbo1a,paocbo1a)*OCBO1 + c(paocbo2a,paocbo2a)*OCBO2 + c(paocbo3a,paocbo3a)*OCBO3 + c(paocbo4a,paocbo4a)*OCBO4 + c(paocbo5a,paocbo5a)*OCBO5 + c(paocbo6a,paocbo6a)*OCBO6 + c(paocbo7a,paocbo7a)*OCBO7
PA ~~ ((1-.921)*0.824)*PA
...
OCBO ~ c(0, 0)*1
'
methoduc <- cfa(methoduc.cfa, ordered = c("PP1", "PP2", "PP3", 
                                              "PP4", "PP5", "PP6", "PP7", "PP8", "PP9", "PP10", 
                                              "IRB1", "IRB2", "IRB3", "IRB4", "IRB5", "IRB6", "IRB7", "OCBI1",
                                              "OCBI2", "OCBI3", "OCBI4", "OCBI5", "OCBI6", "OCBI7", "OCBO1", 
                                              "OCBO2", "OCBO3", "OCBO4", "OCBO5", "OCBO6", "OCBO7"),
                  group = "COND", data = data1, estimator = "DWLS", parameterization = "theta", information = "expected", std.lv=TRUE)
anova(methoduc,methodu) #Method variance attributable to positive affectivity and negative item wording varies across the two conditions. 
...
OCBO ~ c(0, 0)*1
'
methodup <- cfa(methodup.cfa, ordered = c("PP1", "PP2", "PP3", 
                                              "PP4", "PP5", "PP6", "PP7", "PP8", "PP9", "PP10", 
                                              "IRB1", "IRB2", "IRB3", "IRB4", "IRB5", "IRB6", "IRB7", "OCBI1",
                                              "OCBI2", "OCBI3", "OCBI4", "OCBI5", "OCBI6", "OCBI7", "OCBO1", 
                                              "OCBO2", "OCBO3", "OCBO4", "OCBO5", "OCBO6", "OCBO7"),
...
#Model 7: Method UPI model. Allow substantive factor loadings to vary across conditions. Additionally, (1) force the differences in factor loadings to be equal within factors but to (2) vary across factors. Tests the hypothesis that proximal causes of method effects addressed by proximal remedies are congeneric (vs. non-congeneric). If this model is not statistically different from method u, then proximal method effects are congeneric and so a simple correction for failing to use proximal remedies will not be viable. Therefore, proximal method effects are more complex than considered previously. 
methodupi.cfa <- '
...
OCBO ~ c(0, 0)*1
'
methodupi <- cfa(methodupi.cfa, ordered = c("PP1", "PP2", "PP3", 
                                              "PP4", "PP5", "PP6", "PP7", "PP8", "PP9", "PP10", 
                                              "IRB1", "IRB2", "IRB3", "IRB4", "IRB5", "IRB6", "IRB7", "OCBI1",
                                              "OCBI2", "OCBI3", "OCBI4", "OCBI5", "OCBI6", "OCBI7", "OCBO1", 
                                              "OCBO2", "OCBO3", "OCBO4", "OCBO5", "OCBO6", "OCBO7"),
...
##Factor covariances freely estimated (with the exception being the bifactors). However, correlations with the method factors are fixed to zero. Method factors are allowed to covary.
PP ~~ c(.358,.379)*IRB
PP ~~ c(.536,.553)*OCBI
PP ~~ c(.498,.382)*OCBO
PP ~~ c(0,0)*PosAff
...
OCBO ~ c(0, 0)*1
'
methodru <- cfa(methodru.cfa, ordered = c("PP1", "PP2", "PP3", 
                                              "PP4", "PP5", "PP6", "PP7", "PP8", "PP9", "PP10", 
                                              "IRB1", "IRB2", "IRB3", "IRB4", "IRB5", "IRB6", "IRB7", "OCBI1",
                                              "OCBI2", "OCBI3", "OCBI4", "OCBI5", "OCBI6", "OCBI7", "OCBO1", 
                                              "OCBO2", "OCBO3", "OCBO4", "OCBO5", "OCBO6", "OCBO7"),
                  group = "COND", data = data1, estimator = "DWLS", parameterization = "theta", information = "expected", std.lv=TRUE)
anova(methodru,methodu) #Method bias attributable to positive affectivity and proximal causes of method variance has been detected. Unfortunately, this test is also sensitive to inflation that has occurred in the criterion variables and so we must account for this. Need to determine which relationships specifically have been strongly affected by sources of method variance.
...
#Constrain the factor covariance differences to be equal within factors but vary across factors. Constrain the between-condition factor covariances that are statistically indistinguishable from zero (p > .05; as identified by the best-fitting model, which in this case is the Method U model) to be equal. This helps to see how complex method variance is. For this particular model, only the IRB~OCBI and IRB~OCBO relationships seemingly affected.
PPIRB1==PPIRB2
PPOCBI1==PPOCBI2
PPOCBO1==PPOCBO2
OCBIO1==OCBIO2
...
OCBO ~ c(0, 0)*1
'
methoduu <- cfa(methoduu.cfa, ordered = c("PP1", "PP2", "PP3", 
                                              "PP4", "PP5", "PP6", "PP7", "PP8", "PP9", "PP10", 
                                              "IRB1", "IRB2", "IRB3", "IRB4", "IRB5", "IRB6", "IRB7", "OCBI1",
                                              "OCBI2", "OCBI3", "OCBI4", "OCBI5", "OCBI6", "OCBI7", "OCBO1", 
                                              "OCBO2", "OCBO3", "OCBO4", "OCBO5", "OCBO6", "OCBO7"),
                  group = "COND", data = data1, estimator = "DWLS", parameterization = "theta", information = "expected", std.lv=TRUE)
anova(initial, methodu, methodi, methoduc, methodup, methodupi, methoduu, methodru) #Interestingly, the focal coefficients of interest (PP~IRB, OCBI, OCBO) were invariant across groups. In other words, though proximal remedies were used to reduce common method variance in the focal predictor-criterion relationships and they appeared to cause differences in substantive factor loadings, they did not cause differences in the focal substantive parameter estimates. This may suggest that proximal causes of CMV are not be powerful enough alone to cause substantial amounts of bias. By contrast, the method bias that was observed in this study was attributable to positive affectivity. 
...
method <- cfa(model = method.cfa, ordered = c("PA1", "PA2", "PA3", "PA4", "PA5", "PA6", "PA7", "PA8", "PA9", "PA10", 
                                      "NA1", "NA2", "NA3", "NA4", "NA5", "NA6", "NA7", "NA8", "NA9", "NA10", 
                                      "CM1", "CM2", "CM3", "CM4", "CM5", "CM6", "CM7", "CM8"), 
                                      data = data2, parameterization = "theta", estimator = "dwls", information = "expected")
#Get method factor scores
df <- predict(method)
...
OCBO =~ c(ocbo1a,ocbo1b,ocbo1c,ocbo1d,ocbo1e)*OCBO1 + c(ocbo2a,ocbo2b,ocbo2c,ocbo2d,ocbo2e)*OCBO2 + c(ocbo3a,ocbo3b,ocbo3c,ocbo3d,ocbo3e)*OCBO3 + c(ocbo4a,ocbo4b,ocbo4c,ocbo4d,ocbo4e)*OCBO4 + c(ocbo5a,ocbo5b,ocbo5c,ocbo5d,ocbo5e)*OCBO5 + c(ocbo6a,ocbo6b,ocbo7c,ocbo7d,ocbo7e)*OCBO6 + c(ocbo7a,ocbo7b,ocbo7c,ocbo7d,ocbo7e)*OCBO7
...
initial <- cfa(initial.cfa, ordered = c("PP1", "PP2", "PP3", 
                                              ...
                                              "OCBI2", "OCBI3", "OCBI4", "OCBI5", "OCBI6", "OCBI7", "OCBO1", 
                                              "OCBO2", "OCBO3", "OCBO4", "OCBO5", "OCBO6", "OCBO7"),
                  group = "COND", data = data2, estimator = "DWLS", parameterization = "theta", information = "expected", std.lv=TRUE)
...
##Method factors
PosAff =~ c(1,1,1,1,1)*PA + c(papp1a, papp1b,papp1c,papp1d,papp1e)*PP1 + c(papp2a, papp2b,papp2c,papp2d,papp2e)*PP2 + c(papp3a,papp3b,papp3c,papp3d,papp3e)*PP3 + c(papp4a,papp4b,papp4c,papp4d,papp4e)*PP4 + c(papp5a,papp5b,papp5c,papp5d,papp5e)*PP5 + c(papp6a,papp6b,papp6c,papp6d,papp6e)*PP6 + c(papp7a,papp7b,papp7c,papp7d,papp7e)*PP7 + c(papp8a,papp8b,papp8c,papp8d,papp8e)*PP8 + c(papp9a,papp9b,papp9c,papp9d,papp9e)*PP9 + c(papp10a,papp10b,papp10c,papp10d,papp10e)*PP10 + c(pairb1a,pairb1b,pairb1c,pairb1d,pairb1e)*IRB1 + c(pairb2a,pairb2b,pairb2c,pairb2d,pairb2e)*IRB2 + c(pairb3a,pairb3b,pairb3c,pairb3d,pairb3e)*IRB3 + c(pairb4a,pairb4b,pairb4c,pairb4d,pairb4e)*IRB4 + c(pairb5a,pairb5b,pairb5c,pairb5d,pairb5e)*IRB5 + c(pairb6a,pairb6b,pairb6c,pairb6d,pairb6e)*IRB6 + c(pairb7a,pairb7b,pairb7c,pairb7d,pairb7e)*IRB7 + c(paocbi1a,paocbi1b,paocbi1c,paocbi1d,paocbi1e)*OCBI1 + c(paocbi2a,paocbi2b,paocbi2c,paocbi2d,paocbi2e)*OCBI2 + c(paocbi3a,paocbi3b,paocbi3c,paocbi3d,paocbi3e)*OCBI3 + c(paocbi4a,paocbi4b,paocbi4c,paocbi4d,paocbi4e)*OCBI4 + c(paocbi5a,paocbi5b,paocbi5c,paocbi5d,paocbi5e)*OCBI5 + c(paocbi6a,paocbi6b,paocbi6c,paocbi6d,paocbi6e)*OCBI6 + c(paocbi7a,paocbi7b,paocbi7c,paocbi7d,paocbi7e)*OCBI7 + c(paocbo1a,paocbo1b,paocbo1c,paocbo1d,paocbo1e)*OCBO1 + c(paocbo2a,paocbo2b,paocbo2c,paocbo2d,paocbo2e)*OCBO2 + c(paocbo3a,paocbo3b,paocbo3c,paocbo3d,paocbo3e)*OCBO3 + c(paocbo4a,paocbo4b,paocbo4c,paocbo4d,paocbo4e)*OCBO4 + c(paocbo5a,paocbo5b,paocbo5c,paocbo5d,paocbo5e)*OCBO5 + c(paocbo6a,paocbo6b,paocbo6c,paocbo6d,paocbo6e)*OCBO6 + c(paocbo7a,paocbo7b,paocbo7c,paocbo7d,paocbo7e)*OCBO7
PA ~~ ((1-.925)*0.4259783)*PA
...
ocbo3ab := ocbo3a-ocbo3b
ocbo3ac := ocbo3a-ocbo3c
ocbo3ad := ocbo3a-ocbo3d
ocbo3ae := ocbo3a-ocbo3e
ocbo3bc := ocbo3b-ocbo3c
ocbo3bd := ocbo3b-ocbo3d
ocbo3be := ocbo3b-ocbo3e
ocbo3cd := ocbo3c-ocbo3d
ocbo3ce := ocbo3c-ocbo3e
ocbo3de := ocbo3d-ocbo3e
ocbo4ab := ocbo4a-ocbo4b
ocbo4ac := ocbo4a-ocbo4c
ocbo4ad := ocbo4a-ocbo4d
ocbo4ae := ocbo4a-ocbo4e
ocbo4bc := ocbo4b-ocbo4c
ocbo4bd := ocbo4b-ocbo4d
ocbo4be := ocbo4b-ocbo4e
ocbo4cd := ocbo4c-ocbo4d
ocbo4ce := ocbo4c-ocbo4e
ocbo4de := ocbo4d-ocbo4e
ocbo5ab := ocbo5a-ocbo5b
ocbo5ac := ocbo5a-ocbo5c
ocbo5ad := ocbo5a-ocbo5d
ocbo5ae := ocbo5a-ocbo5e
ocbo5bc := ocbo5b-ocbo5c
ocbo5bd := ocbo5b-ocbo5d
ocbo5be := ocbo5b-ocbo5e
ocbo5cd := ocbo5c-ocbo5d
ocbo5ce := ocbo5c-ocbo5e
ocbo5de := ocbo5d-ocbo5e
...
pairb5ab := pairb5a-pairb5b
pairb5ac := pairb5a-pairb5c
pairb5ad := pairb5a-pairb5d
pairb5ae := pairb5a-pairb5e
pairb5bc := pairb5b-pairb5c
pairb5bd := pairb5b-pairb5d
pairb5be := pairb5b-pairb5e
pairb5cd := pairb5c-pairb5d
pairb5ce := pairb5c-pairb5e
pairb5de := pairb5d-pairb5e
pairb6ab := pairb6a-pairb6b
pairb6ac := pairb6a-pairb6c
pairb6ad := pairb6a-pairb6d
pairb6ae := pairb6a-pairb6e
pairb6bc := pairb6b-pairb6c
pairb6bd := pairb6b-pairb6d
pairb6be := pairb6b-pairb6e
pairb6cd := pairb6c-pairb6d
pairb6ce := pairb6c-pairb6e
pairb6de := pairb6d-pairb6e
pairb7ab := pairb7a-pairb7b
pairb7ac := pairb7a-pairb7c
pairb7ad := pairb7a-pairb7d
pairb7ae := pairb7a-pairb7e
pairb7bc := pairb7b-pairb7c
pairb7bd := pairb7b-pairb7d
pairb7be := pairb7b-pairb7e
pairb7cd := pairb7c-pairb7d
pairb7ce := pairb7c-pairb7e
pairb7de := pairb7d-pairb7e
...
paocbo3ab := paocbo3a-paocbo3b
paocbo3ac := paocbo3a-paocbo3c
paocbo3ad := paocbo3a-paocbo3d
paocbo3ae := paocbo3a-paocbo3e
paocbo3bc := paocbo3b-paocbo3c
paocbo3bd := paocbo3b-paocbo3d
paocbo3be := paocbo3b-paocbo3e
paocbo3cd := paocbo3c-paocbo3d
paocbo3ce := paocbo3c-paocbo3e
paocbo3de := paocbo3d-paocbo3e
paocbo4ab := paocbo4a-paocbo4b
paocbo4ac := paocbo4a-paocbo4c
paocbo4ad := paocbo4a-paocbo4d
paocbo4ae := paocbo4a-paocbo4e
paocbo4bc := paocbo4b-paocbo4c
paocbo4bd := paocbo4b-paocbo4d
paocbo4be := paocbo4b-paocbo4e
paocbo4cd := paocbo4c-paocbo4d
paocbo4ce := paocbo4c-paocbo4e
paocbo4de := paocbo4d-paocbo4e
paocbo5ab := paocbo5a-paocbo5b
paocbo5ac := paocbo5a-paocbo5c
paocbo5ad := paocbo5a-paocbo5d
paocbo5ae := paocbo5a-paocbo5e
paocbo5bc := paocbo5b-paocbo5c
paocbo5bd := paocbo5b-paocbo5d
paocbo5be := paocbo5b-paocbo5e
paocbo5cd := paocbo5c-paocbo5d
paocbo5ce := paocbo5c-paocbo5e
paocbo5de := paocbo5d-paocbo5e
...
##Method factors
##Substantive factors
PosAff =~ c(1,1,1,1,1)*PA + c(papp1a, papp1a,papp1a,papp1a,papp1a)*PP1 + c(papp2a, papp2a,papp2a,papp2a,papp2a)*PP2 + c(papp3a,papp3a,papp3a,papp3a,papp3a)*PP3 + c(papp4a,papp4a,papp4a,papp4a,papp4a)*PP4 + c(papp5a,papp5a,papp5a,papp5a,papp5a)*PP5 + c(papp6a,papp6a,papp6a,papp6a,papp6a)*PP6 + c(papp7a,papp7a,papp7a,papp7a,papp7a)*PP7 + c(papp8a,papp8a,papp8a,papp8a,papp8a)*PP8 + c(papp9a,papp9a,papp9a,papp9a,papp9a)*PP9 + c(papp10a,papp10a,papp10a,papp10a,papp10a)*PP10 + c(pairb1a,pairb1a,pairb1a,pairb1a,pairb1a)*IRB1 + c(pairb2a,pairb2a,pairb2a,pairb2a,pairb2a)*IRB2 + c(pairb3a,pairb3a,pairb3a,pairb3a,pairb3a)*IRB3 + c(pairb4a,pairb4a,pairb4a,pairb4a,pairb4a)*IRB4 + c(pairb5a,pairb5a,pairb5a,pairb5a,pairb5a)*IRB5 + c(pairb6a,pairb6a,pairb6a,pairb6a,pairb6a)*IRB6 + c(pairb7a,pairb7a,pairb7a,pairb7a,pairb7a)*IRB7 + c(paocbi1a,paocbi1a,paocbi1a,paocbi1a,paocbi1a)*OCBI1 + c(paocbi2a,paocbi2a,paocbi2a,paocbi2a,paocbi2a)*OCBI2 + c(paocbi3a,paocbi3a,paocbi3a,paocbi3a,paocbi3a)*OCBI3 + c(paocbi4a,paocbi4a,paocbi4a,paocbi4a,paocbi4a)*OCBI4 + c(paocbi5a,paocbi5a,paocbi5a,paocbi5a,paocbi5a)*OCBI5 + c(paocbi6a,paocbi6a,paocbi6a,paocbi6a,paocbi6a)*OCBI6 + c(paocbi7a,paocbi7a,paocbi7a,paocbi7a,paocbi7a)*OCBI7 + c(paocbo1a,paocbo1a,paocbo1a,paocbo1a,paocbo1a)*OCBO1 + c(paocbo2a,paocbo2a,paocbo2a,paocbo2a,paocbo2a)*OCBO2 + c(paocbo3a,paocbo3a,paocbo3a,paocbo3a,paocbo3a)*OCBO3 + c(paocbo4a,paocbo4a,paocbo4a,paocbo4a,paocbo4a)*OCBO4 + c(paocbo5a,paocbo5a,paocbo5a,paocbo5a,paocbo5a)*OCBO5 + c(paocbo6a,paocbo6a,paocbo6a,paocbo6a,paocbo6a)*OCBO6 + c(paocbo7a,paocbo7a,paocbo7a,paocbo7a,paocbo7a)*OCBO7
PA ~~ ((1-.925)*0.4259783)*PA
...
IRB =~ c(irb1a,irb1a,irb1a,irb1a,irb1a)*IRB1 + c(irb2a,irb2a,irb2a,irb2a,irb2a)*IRB2 + c(irb3a,irb3a,irb3a,irb3a,irb3a)*IRB3 + c(irb4a,irb4a,irb4a,irb4a,irb4a)*IRB4 + c(irb5a,irb5a,irb5a,irb5a,irb5a)*IRB5 + c(irb6a,irb6a,irb6a,irb6a,irb6a)*IRB6 + c(irb7a,irb7a,irb7a,irb7a,irb7a)*IRB7
...
OCBO =~ c(ocbo1a,ocbo1a,ocbo1a,ocbo1a,ocbo1a)*OCBO1 + c(ocbo2a,ocbo2a,ocbo2a,ocbo2a,ocbo2a)*OCBO2 + c(ocbo3a,ocbo3a,ocbo3a,ocbo3a,ocbo3a)*OCBO3 + c(ocbo4a,ocbo4a,ocbo4a,ocbo4a,ocbo4a)*OCBO4 + c(ocbo5a,ocbo5a,ocbo5a,ocbo5a,ocbo5a)*OCBO5 + c(ocbo6a,ocbo6a,ocbo6a,ocbo6a,ocbo6a)*OCBO6 + c(ocbo7a,ocbo7a,ocbo7a,ocbo7a,ocbo7e)*OCBO7
...
PP ~~ c(.574,.475,.556,.570,.533)*IRB
PP ~~ c(.724,.676,.524,.343,.589)*OCBI
PP ~~ c(.720,.598,.607,.470,.562)*OCBO
PP ~~ c(0,0,0,0,0)*PosAff
IRB ~~ c(.715,.641,.657,.645,.706)*OCBI
IRB ~~ c(.923,.929,.932,.808,.840)*OCBO
IRB ~~ c(0,0,0,0,0)*PosAff
OCBI ~~ c(.766,.679,.709,.713,.727)*OCBO
OCBI ~~ c(0,0,0,0,0)*PosAff
OCBO ~~ c(0,0,0,0,0)*PosAff
...
#Note: certain response categories are too few for the estimator to work. Specifically, the "strongly disagree" and "disagree" response options were simply combined to reflect a "disagree" option. So I combined them:
data3$IRB1[data3$IRB1==1] <- 2
data3$IRB2[data3$IRB2==1] <- 2
data3$IRB4[data3$IRB4==1] <- 2
data3$OCBI7[data3$OCBI7==1] <- 2
data3$OCBO1[data3$OCBO1==1] <- 2
data3$OCBO4[data3$OCBO4==0] <- 1
data3$OCBO7[data3$OCBO7==6] <- 5
data3$IRB3[data3$IRB3==1] <- 2
data3$IRB3[data3$IRB7==1] <- 2
data3$PP2[data3$PP2==1] <- 2
data3$PP4[data3$PP4==1] <- 2
data3$PP5[data3$PP5==1] <- 2
...
method <- cfa(model = method.cfa, ordered = c("PA1", "PA2", "PA3", "PA4", "PA5", "PA6", "PA7", "PA8", "PA9", "PA10", 
                                      "NA1", "NA2", "NA3", "NA4", "NA5", "NA6", "NA7", "NA8", "NA9", "NA10", 
                                      "CM1", "CM2", "CM3", "CM4", "CM5", "CM6", "CM7", "CM8"), 
                                      data = data3, parameterization = "theta", estimator = "dwls", information = "expected")
#Get method factor scores
df <- predict(method)
