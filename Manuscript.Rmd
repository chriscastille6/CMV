...
##Our Overarching Approach to Examining the Efficacy of Procedural Remedies
  We examined how parameter estimates for a substantive measurement model (i.e., a model containing measures of proactive personality, in-role behavior, and organizational citizenship) were affected by a measured cause of method variance (i.e., positive affectivty). Additionally, we randomoly assigned individuals to receive a non-remedied survey or a survey that was designed to remove any bias in the estimated relationships between between proactive personality and both in-role behavior and organizational citizenship. 
   To study the efficacy of procedural remedies for method variance, we drew upon latent variable modeling strategies proposed by Wiliams and colleagues [e.g., @WilliamsFourResearchDesigns2016; @WilliamsMethodVarianceMarker2010], but adapted these strategies to a multiple group context whereby the efficacy of procedural remedies for method variance was tested. Specifically, we followed a step-wise nested model comparison approach that involved tested key hypotheses regarding the efficacy of procedural remedies for method variance. In our initial step [Phase 1a], we simply examined whether the measures for each of these factors loaded on their respective factors. We also examined model data fit. Afterward [Phase 1b], we moved on to a baseline model whereby the latent covariances linking positive affectivity to the substantive factors were fixed to zero. Theoretical independendence from other latent factors has been considered a requirement linking method factors to substantive factors [see @ConwayWhatReviewersShould2010; @WilliamsIdealnonidealnomarker2015]. In other words, our model testing assumes that positive affectivity contaminates responses to proactive personality, in-role behavior, and ocb [@SpectorMethodologicalUrbanLegends2011]. While this model is compared to the initial model, rejection of this baseline model is not important for our purposes [see @WilliamsFourResearchDesigns2016; @WilliamsMethodVarianceMarker2010]. We then move to a series of model tests whereby positive affectivity and other proximal method effects are assumed to play some contaminating role within our substantive measurement model [Phase 1c]. If these models are superior to the baseline, then method variance in some form is present. After identifying the nature of the method variance [congeneric vs. non-congeneric; see @RichardsonTaleThreePerspectives2009; @WilliamsFourResearchDesigns2016], we then examine whether there is method bias attributable to captured sources of method variance [Phase 1d], which is done by fixing substantive factor correlations to values from the baseline model and testing for a significant difference. If this model is statistical significant from a model developed in Phase 1c, then method bias has occurred [see @WilliamsFourResearchDesigns2016; @WilliamsMethodVarianceMarker2010].
###General Purpose
...
my.keys.list <- list(PP=c("PP1","PP2","PP3","PP4","PP5","PP6","PP7","PP8","PP9","PP10"),
                     IRB=c("IRB1","IRB2","IRB3","IRB4"), #Negatively worded items and item 5 removed)
                     OCBI=c("OCBI1","OCBI2","OCBI3","OCBI4","OCBI5","OCBI6","OCBI7"),
                     OCBO=c("OCBO1","OCBO2","OCBO6","OCBO7"), #Negatively worded items
...
```{r Method Factor Analysis, include=FALSE}
method.cfa <- '
#Method factors
...
#Results - Study 1
```{r Analyses to distribute, include = FALSE}
...
table4 <- table
```
```{r Descriptives Study 1, include = FALSE}
#Create Descriptives Table
scales <- my.scales[["scores"]]
scales <- cbind(data1[c(1,2,4,5)],scales)
scales <- scales[c(1:9)]
table1 <- apa.cor.table(scales, table.number = 1, landscape = FALSE)
```
```{r Table 1, include = TRUE}
table1 <- apa.cor.table(scales)
```
  Descriptive statistics and bivariate correlations can be found in table 1. As expected, proactive personality was moderately and positively related to in-role behavior (*r* = .43), OCBI (*r* = .50),and OCBO (*r* = .47). It was also strongly related to our method factor, positive affectivity. 
#Phase 1a: Evaluation of the Measurement Model
```{r Phase 1a Study 1, include = FALSE}
...
#Relable table to table2.
table2 <- table

#Extract relevant values
chi.initial <- table2$`χ^2`[1]
df.initial <- table2$df[1] 
p.initial <- table2$`p value`[1]
cfi.initial <- table2$CFI[1]
rmsea.initial <- table2$RMSEA[1]
rmseal.initial <- table2$`90% LCL`[1]
rmseau.initial <- table2$`90% UCL`[1]
srmr.initial <- table2$SRMR[1]
```
  We first estimated a CFA measurement model with our three substantive variables and our measured cause variable (positive affectivity). The fit of the measurement model was good: 
chi(`r apa(df.initial,2,T)`) = `r apa(chi.initial,2,T)`, *p* = `r apa(p.initial,2,T)`; CFI = `r apa(cfi.initial,2,T)`; RMSEA = `r apa(rmsea.initial,2,T)` (`r apa(rmseal.initial,2,T)`, `r apa(rmseau.initial,2,T)`); SRMR = `r apa(srmr.initial,2,T)`. None of the factor factor correlations were so high as to warrant concerns about discimininant validity [see Table 1; none exceeded .80; see @brownConfirmatoryFactorAnalysis2006]. 
#Phase 1b: Establishing the Baseline Model
```{r Phase 1b Study 1, include = FALSE}
...
##Factor covariances freely estimated. However, correlations with the method factors are fixed to zero. 
...
#Insert stats
##Chi-Square
table2[2,2]<- round(as.numeric(fitmeasures(baseline)[3]), digits = 2)

##df
table2[2,3]<- round(as.numeric(fitmeasures(baseline)[4]), digits = 2)

##Chi-Square p value
table2[2,4]<- pvalr(as.numeric(fitmeasures(baseline)[5]), digits = 2)

#CFI
table2[2,5]<- round(as.numeric(fitmeasures(baseline)[9]), digits = 3)

##RMSEA
table2[2,6]<- round(as.numeric(fitmeasures(baseline)[17]), digits = 3)

#RMSEA lower
table2[2,7]<- round(as.numeric(fitmeasures(baseline)[18]), digits = 3)

#RMSEA upper
table2[2,8]<- round(as.numeric(fitmeasures(baseline)[19]), digits = 3)

#RMSEA p-value
table2[2,9]<- pvalr(as.numeric(fitmeasures(baseline)[20]), digits = 3)

#SRMR
table2[2,10]<- round(as.numeric(fitmeasures(baseline)[23]), digits = 3)

#Extract relevant statistics
cfi.baseline <- table2$`χ^2`[2]
df.baseline <- table2$df[2] 
p.baseline <- table2$`p value`[2]
cfi.baseline <- table2$CFI[2]
rmsea.baseline <- table2$RMSEA
rmseal.baseline <- table2$`90% LCL`[2]
rmseau.baseline <- table2$`90% UCL`[2]
srmr.baseline <- table2$SRMR[2]
```
  Having established our initial measurement model, we moved to a baseline model whereby the factor correlations linking positive affectivity to our substantive factors was fixed to zero. This is done because theoretical independendence from other latent factors has been considered a requirement linking method factors to substantive factors [see @ConwayWhatReviewersShould2010; @WilliamsIdealnonidealnomarker2015]. The fit of this model was, as expected, poorer than the baseline model: chi(`r apa(df.baseline,2,T)`) = `r apa(chi.baseline,2,T)`, *p* = `r apa(p.baseline,2,T)`; CFI = `r apa(cfi.baseline,2,T)`; RMSEA = `r apa(rmsea.baseline,2,T)` (`r apa(rmseal.baseline,2,T)`, `r apa(rmseau.baseline,2,T)`); SRMR = `r apa(srmr.baseline,2,T)`. We then moved to test for the presence and nature of method variance attributable to positive affectivity and proximal causes.
#Phase 1c: Test for the Presence and Nature of Method Variance
```{r Phase 1c Step 1 Study 1, include = FALSE}
#Model 3: Method U model. Freely estimate method effects across groups. Tests for the pressence of method effects attributable to positive affectivity. The positive affectivity method effects are allowed to vary across conditions. Factor differences in factor loadings and covariances have been computed to facilitate interpretation of method effects. 
methodu.cfa <- '
...
##Factor covariances freely estimated. However, correlations with the method factors are fixed to zero. Method factors are also constrained to zero.
PP ~~ c(PPIRB1,PPIRB2)*IRB
PP ~~ c(PPOCBI1,PPOCBI2)*OCBI
PP ~~ c(PPOCBO1,PPOCBO2)*OCBO
PP ~~ c(0,0)*PosAff
...
#Insert stats
##Chi-Square
table2[3,2]<- round(as.numeric(fitmeasures(methodu)[3]), digits = 2)

##df
table2[3,3]<- round(as.numeric(fitmeasures(methodu)[4]), digits = 2)

##Chi-Square p value
table2[3,4]<- pvalr(as.numeric(fitmeasures(methodu)[5]), digits = 2)

#CFI
table2[3,5]<- round(as.numeric(fitmeasures(methodu)[9]), digits = 3)

##RMSEA
table2[3,6]<- round(as.numeric(fitmeasures(methodu)[17]), digits = 3)

#RMSEA lower
table2[3,7]<- round(as.numeric(fitmeasures(methodu)[18]), digits = 3)

#RMSEA upper
table2[3,8]<- round(as.numeric(fitmeasures(methodu)[19]), digits = 3)

#RMSEA p-value
table2[3,9]<- pvalr(as.numeric(fitmeasures(methodu)[20]), digits = 3)

#SRMR
table2[3,10]<- round(as.numeric(fitmeasures(methodu)[23]), digits = 3)

#Extract relevant statistics
cfi.methodu <- table2$`χ^2`[3]
df.methodu <- table2$df[3] 
p.methodu <- table2$`p value`[3]
cfi.methodu <- table2$CFI[3]
rmsea.methodu <- table2$RMSEA
rmseal.methodu <- table2$`90% LCL`[3]
rmseau.methodu <- table2$`90% UCL`[3]
srmr.methodu <- table2$SRMR[3]
...
#Change table to table 3.
table3 <- table

#Extract relevant statistics.
chi.sq.diff.methodu.b <- table3$χ[2]
pval.chi.sq.diff.methodu.b <- table3$`p value`[2]
```
  In phase 1c, we conducted a series of sequential model comparisons. We started first with positive affectivity given its strong relationship to our substantive variables of interest but then moved to imposing constraints that tested for toerh proximal effects on substantive factor loadings. For step 1 of this phase, we added method factor loadings (MFLs) from positive affectivity to the substantive indicators and freely estimated these paths. In other words, this model [Method-U; see @williamsFourResearchDesigns2015] tests for the pressence of method effects attributable to a common sources of variance (proximal causes and positive affectivity). Method effects were allowed to vary across conditions. The fit of this model was good: chi(`r apa(df.methodu,2,T)`) = `r apa(chi.methodu,2,T)`, *p* = `r apa(p.methodu,2,T)`; CFI = `r apa(cfi.methodu,2,T)`; RMSEA = `r apa(rmsea.methodu,2,T)` (`r apa(rmseal.methodu,2,T)`, `r apa(rmseau.methodu,2,T)`); SRMR = `r apa(srmr.methodu,2,T)`. Because this model was superior to the baseline model (chi.sq.diff = `r apa(chi.sq.diff.methodu.b,2,T)`, *p* < `r apa(pval.chi.sq.diff.methodu.b,2,T)`), we retained it. It is worth noting that 15/25 items (60%) were contaminated by positive affectivity (*p* < .05). 
```{r Phase 1c Step 2 Study 1, include = FALSE}
...
anova(methodi,methodu) #Congeneric method variance is present. 
...
#Insert stats
##Chi-Square
table2[4,2]<- round(as.numeric(fitmeasures(methodi)[3]), digits = 2)

##df
table2[4,3]<- round(as.numeric(fitmeasures(methodi)[4]), digits = 2)

##Chi-Square p value
table2[4,4]<- pvalr(as.numeric(fitmeasures(methodi)[5]), digits = 2)

#CFI
table2[4,5]<- round(as.numeric(fitmeasures(methodi)[9]), digits = 3)

##RMSEA
table2[4,6]<- round(as.numeric(fitmeasures(methodi)[17]), digits = 3)

#RMSEA lower
table2[4,7]<- round(as.numeric(fitmeasures(methodi)[18]), digits = 3)

#RMSEA upper
table2[4,8]<- round(as.numeric(fitmeasures(methodi)[19]), digits = 3)

#RMSEA p-value
table2[4,9]<- pvalr(as.numeric(fitmeasures(methodi)[20]), digits = 3)

#SRMR
table2[4,10]<- round(as.numeric(fitmeasures(methodi)[23]), digits = 3)

#Extract relevant statistics
cfi.methodi <- table2$`χ^2`[4]
df.methodi <- table2$df[4] 
p.methodi <- table2$`p value`[4]
cfi.methodi <- table2$CFI[4]
rmsea.methodi <- table2$RMSEA
rmseal.methodi <- table2$`90% LCL`[4]
rmseau.methodi <- table2$`90% UCL`[4]
srmr.methodi <- table2$SRMR[4]

#Insert stats
#Chi-square difference
table3[2,2]<- abs(table2$`χ^2`[3] - table2$`χ^2`[4])

#df difference
table3[2,3]<- abs(table2$df[3] - table2$df[4])

#chi-square critical value
table3[2,4]<- round(qchisq(.05, table$df[2], lower.tail=FALSE), digits = 2)

#p value
table3[2,5]<- pvalr(pchisq(table$χ[2], table$df[2], lower.tail=FALSE), digits = 2)

#Extract relevant statistics.
chi.sq.diff.methodu.i <- table3$χ[3]
pval.chi.sq.diff.methodu.i <- table3$`p value`[3]
```
  We then constrained estimates of method effects attributable to positive affectivity to be equal within substantive variables but to vary across substantive variables [Method-i; see @williamsFourResearchDesigns2015]. This tests whether method effects attributable to positive affectivity are congeneric or non-congeneric [see @richardsonTaleThreePerspectives2009]. While the fit of this model was good in absolute terms, it was relatively poorer compared to Method-U (chi.sq.diff = `r apa(chi.sq.diff.methodu.i,2,T)`, *p* < `r apa(pval.chi.sq.diff.methodu.i,2,T)`), further supporting the notion that contamination due to positive affectivity is congeneric.
```{r Phase 1c Step 3 Study 1, include = FALSE}
...
#Insert stats
##Chi-Square
table2[5,2]<- round(as.numeric(fitmeasures(methoduc)[3]), digits = 2)

##df
table2[5,3]<- round(as.numeric(fitmeasures(methoduc)[4]), digits = 2)

##Chi-Square p value
table2[5,4]<- pvalr(as.numeric(fitmeasures(methoduc)[5]), digits = 2)

#CFI
table2[5,5]<- round(as.numeric(fitmeasures(methoduc)[9]), digits = 3)

##RMSEA
table2[5,6]<- round(as.numeric(fitmeasures(methoduc)[17]), digits = 3)

#RMSEA lower
table2[5,7]<- round(as.numeric(fitmeasures(methoduc)[18]), digits = 3)

#RMSEA upper
table2[5,8]<- round(as.numeric(fitmeasures(methoduc)[19]), digits = 3)

#RMSEA p-value
table2[5,9]<- pvalr(as.numeric(fitmeasures(methoduc)[20]), digits = 3)

#SRMR
table2[5,10]<- round(as.numeric(fitmeasures(methoduc)[23]), digits = 3)

#Insert stats
#Chi-square difference
table3[3,2]<- abs(table2$`χ^2`[3] - table2$`χ^2`[5])

#df difference
table3[3,3]<- abs(table2$df[3] - table2$df[5])

#chi-square critical value
table3[3,4]<- round(qchisq(.05, table$df[3], lower.tail=FALSE), digits = 2)

#p value
table3[3,5]<- pvalr(pchisq(table$χ[3], table$df[3], lower.tail=FALSE), digits = 2)

#Extract relevant statistics.
chi.sq.diff.methodu.uc <- table3$χ[4]
pval.chi.sq.diff.methodu.uc <- table3$`p value`[4]
```
  We then constrained estimates of method effects attributable to positive affectivity to be equal across conditions. This is done by forcing the MFLs to be equal across conditions and tests whether method variance attributable to positive affectivity is equally present across conditions (Method-Uc). The results suggested that the model with constrained MFLs was equivalent to the Method-U model that lacked these constraints (chi.sq.diff = `r apa(chi.sq.diff.methodu.uc,2,T)`, *p* < `r apa(pval.chi.sq.diff.methodu.uc,2,T)`). Therefore, equivalence of method variance attributable to positive affectivity was assumed across both conditions. 
```{r Phase 1c Step 4 Study 1, include = FALSE}
...
#Insert stats
##Chi-Square
table2[6,2]<- round(as.numeric(fitmeasures(methodup)[3]), digits = 2)

##df
table2[6,3]<- round(as.numeric(fitmeasures(methodup)[4]), digits = 2)

##Chi-Square p value
table2[6,4]<- pvalr(as.numeric(fitmeasures(methodup)[5]), digits = 2)

#CFI
table2[6,5]<- round(as.numeric(fitmeasures(methodup)[9]), digits = 3)

##RMSEA
table2[6,6]<- round(as.numeric(fitmeasures(methodup)[17]), digits = 3)

#RMSEA lower
table2[6,7]<- round(as.numeric(fitmeasures(methodup)[18]), digits = 3)

#RMSEA upper
table2[6,8]<- round(as.numeric(fitmeasures(methodup)[19]), digits = 3)

#RMSEA p-value
table2[6,9]<- pvalr(as.numeric(fitmeasures(methodup)[20]), digits = 3)

#SRMR
table2[6,10]<- round(as.numeric(fitmeasures(methodup)[23]), digits = 3)

#Insert stats
#Chi-square difference
table3[4,2]<- abs(table2$`χ^2`[3] - table2$`χ^2`[6])

#df difference
table3[4,3]<- abs(table2$df[3] - table2$df[6])

#chi-square critical value
table3[4,4]<- round(qchisq(.05, table$df[4], lower.tail=FALSE), digits = 2)

#p value
table3[4,5]<- pvalr(pchisq(table$χ[4], table$df[4], lower.tail=FALSE), digits = 2)

#Extract relevant statistics.
chi.sq.diff.methoduc.up <- table3$χ[5]
pval.chi.sq.diff.methoduc.up <- table3$`p value`[5]
```
  We then constrained the substantive factor loadings to be equal across conditions (Method-Up). This tests the hypothesis that proximal remedies address the serial-item order effects that is believed to inflate the substantive factor loadings of our measurement model. If this model is equivalent to Method-Uc, then proximal remedies do not cause a meaningful difference in factor loadings. The results suggested that the model with equivalent substantive factor loadings was inferior to the Method-Uc model, which lacked these constraints (chi.sq.diff = `r apa(chi.sq.diff.methoduc.up,2,T)`, *p* < `r apa(pval.chi.sq.diff.methoduc.up,2,T)`). Therefore, it appears as though these non-measured proximal effects were presumably present in the non-remedied condition. It also supports the notion that our proximal remedies address unmeasured proximal causes of method variance. 
```{r Phase 1c Step 5 Study 1, include = FALSE}
...
## extract fit indices tables
fitm.methodupi <- fitmeasures(methodupi)    #Model 6: Constrain substantive factor loadings to be equal across conditions (proximal remedies decontaminate measurement models).

#Insert stats
##Chi-Square
table2[7,2]<- round(as.numeric(fitmeasures(methodupi)[3]), digits = 2)

##df
table2[7,3]<- round(as.numeric(fitmeasures(methodupi)[4]), digits = 2)

##Chi-Square p value
table2[7,4]<- pvalr(as.numeric(fitmeasures(methodupi)[5]), digits = 2)

#CFI
table2[7,5]<- round(as.numeric(fitmeasures(methodupi)[9]), digits = 3)

##RMSEA
table2[7,6]<- round(as.numeric(fitmeasures(methodupi)[17]), digits = 3)

#RMSEA lower
table2[7,7]<- round(as.numeric(fitmeasures(methodupi)[18]), digits = 3)

#RMSEA upper
table2[7,8]<- round(as.numeric(fitmeasures(methodupi)[19]), digits = 3)

#RMSEA p-value
table2[7,9]<- pvalr(as.numeric(fitmeasures(methodupi)[20]), digits = 3)

#SRMR
table2[7,10]<- round(as.numeric(fitmeasures(methodupi)[23]), digits = 3)

#Insert stats
#Chi-square difference
table3[5,2]<- abs(table2$`χ^2`[3] - table2$`χ^2`[6])

#df difference
table3[5,3]<- abs(table2$df[3] - table2$df[6])

#chi-square critical value
table3[5,4]<- round(qchisq(.05, table$df[4], lower.tail=FALSE), digits = 2)

#p value
table3[5,5]<- pvalr(pchisq(table$χ[4], table$df[4], lower.tail=FALSE), digits = 2)

#Extract relevant statistics.
chi.sq.diff.methoduc.up <- table3$χ[6]
pval.chi.sq.diff.methoduc.up <- table3$`p value`[6]
```
   It is worth noting that the Method-Uc model assumes that these effects vary across items. Therefore, we sought to establish whether a simpler assumption (e.g., non-congeneric item-order effects) is tenable. Weran a model whereby the substantive factor loadings we allowed to vary across conditions. However, we forced the differences in factor loadings to be equal within factors and yet vary across factors. Conceptually analogous to Method-I (we termed it Method-Upi), this model tests the hypothesis that proximal causes of method effects addressed by proximal remedies are congeneric (vs. non-congeneric) and scale-specific. This was done by forcing the differences in factor loadings within scales to be equal across conditions. If this model worse from Method-Uc, then proximal method effects vary across items. The results suggested that the model with equivalent substantive factor loadings was inferior to the Method-Uc model, which lacked these constraints (chi.sq.diff = `r apa(chi.sq.diff.methoduc.up,2,T)`, *p* < `r apa(pval.chi.sq.diff.methoduc.up,2,T)`). Therefore, it appears as though the proximal remedies do not address serial-item order effects.
```{r Phase 1c Step 6 Study 1, include = FALSE}
...
## extract fit indices tables
fitm.methodupc <- fitmeasures(methodupc)    #Model 6: Constrain substantive factor loadings to be equal across conditions (proximal remedies decontaminate measurement models).

#Insert stats
##Chi-Square
table2[8,2]<- round(as.numeric(fitmeasures(methodupi)[3]), digits = 2)

##df
table2[8,3]<- round(as.numeric(fitmeasures(methodupi)[4]), digits = 2)

##Chi-Square p value
table2[8,4]<- pvalr(as.numeric(fitmeasures(methodupi)[5]), digits = 2)

#CFI
table2[8,5]<- round(as.numeric(fitmeasures(methodupi)[9]), digits = 3)

##RMSEA
table2[8,6]<- round(as.numeric(fitmeasures(methodupi)[17]), digits = 3)

#RMSEA lower
table2[8,7]<- round(as.numeric(fitmeasures(methodupi)[18]), digits = 3)

#RMSEA upper
table2[8,8]<- round(as.numeric(fitmeasures(methodupi)[19]), digits = 3)

#RMSEA p-value
table2[8,9]<- pvalr(as.numeric(fitmeasures(methodupi)[20]), digits = 3)

#SRMR
table2[8,10]<- round(as.numeric(fitmeasures(methodupi)[23]), digits = 3)

#Insert stats
#Chi-square difference
table3[6,2]<- abs(table2$`χ^2`[3] - table2$`χ^2`[6])

#df difference
table3[6,3]<- abs(table2$df[3] - table2$df[6])

#chi-square critical value
table3[6,4]<- round(qchisq(.05, table$df[4], lower.tail=FALSE), digits = 2)

#p value
table3[6,5]<- pvalr(pchisq(table$χ[4], table$df[4], lower.tail=FALSE), digits = 2)

#Extract relevant statistics.
chi.sq.diff.methoduc.up <- table3$χ[7]
pval.chi.sq.diff.methoduc.up <- table3$`p value`[7]
```
  Having demonstrated that method effects are scale-specific in our study, we then ran a model whereby the differences in factor loadings were constrained to be equal across all factors, which is analogous to Method-C [@WilliamsMethodVarianceMarker2010]. In other words, we tested whether the proximal method effects are a constant across all of our scales. The results suggested that Methodupi is statistically different from methodupc and a better fit. Therefore, a  correction for failing to use a proximal remedy for CMV is viable) or if the correction would have to be measure specific, which is consistent with a measure-centric view of method variance (Spector et al., 2017). *More research is needed to verify this (especially if replicated in study 2).  
```{r Phase 1c Step 7 Study 1, include = FALSE}
...
```{r Phase 1c Step 8 Study 1, include = FALSE}
